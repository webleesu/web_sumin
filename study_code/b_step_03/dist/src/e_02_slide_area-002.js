 ...
 * + 인디케이터 처리 수행 +
 * 5.2.1 클릭시 해당 순번 파악
 * 5.2.2 파악된 광고 슬라이드 해당위치로 이동(공통변수)
 ...
 * 광고 자체 링크는 우선 'tabIndex = -1' 마우스 클릭/ indicator처리 / 다음/ 이전버튼으로 화면이 나타나는 시점에서 'tabIndex=0`
 * indicator기준 클릭이 아닌 focus 처리시 광고화면이 이동, 엔터키를 누르면 해당하는 광고의 위치로 이동
 */
...
(()=>{
  ...
  const aniPrevSlide = async () => {
    ...
    // 일정시간마다 광고슬라이드 이동 수행 (다음 버튼 클릭과 동일한 기능)
    let slideGo;
